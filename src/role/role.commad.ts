...
export class RoleCommand extends CommandRunner {
  ...
  @Option({
    ...
    description: 'IDs de permisos separados por coma',
  })
  ...
  async createRole(options: RoleCommandOptions) {
    console.log('Creando rol con los siguientes datos:', options);
    /*if (options.yes) {
      // modo no interactivo
      if (!options.name || !options.permissions) {
        console.log(
          '❌ Debes especificar al menos --name y --permissions cuando usas --yes.',
        );
        return;
      }
