...
export class PermissionCommand extends CommandRunner {
  ...
  async run(
    ...
    if (options?.create) {
      ...
      console.log(
        '❌ No se especificó ninguna opción. Usa --help para ver los comandos disponibles.',
      );
    }
  }
  ...
  @Option({
    flags: '-y, --yes',
    description: 'Confirma automáticamente sin preguntar',
  })
  parseYes(): boolean {
    return true;
  }
  ...
  async createPermission(options: PermissionCommandOptions) {
    console.log('Creando permiso con los siguientes datos:', options);
    /*if (options.yes) {
      // modo no interactivo
      if (!options.name || !options.code) {
        console.log(
          '❌ Debes especificar al menos --name y --code cuando usas --yes.',
        );
        return;
      }
